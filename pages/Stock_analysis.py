...
st.write(f"**Long Business Summary:** {stock.info.get('longBusinessSummary', 'Not available')}")
st.write(f"**Sector:** {stock.info.get('sector', 'Not available')}")
...
with col1:
    ...
    st.plotly_chart(fig_df, key="table1")
...
with col2:
    ...
    fig_df = plotly_table(df)
    st.plotly_chart(fig_df, key="table2")
...
st.plotly_chart(fig_df, key="table3")
...
if chart_type == "Candle" and indicators == "RSI":
    st.plotly_chart(candlestick(new_df1, num_period), key="chart1")
    st.plotly_chart(RSI(new_df1, num_period), key="chart2")

elif chart_type == "Candle" and indicators == "MACD":
    st.plotly_chart(candlestick(new_df1, num_period), key="chart3")
    st.plotly_chart(MACD(new_df1, num_period), key="chart4")

elif chart_type == "Line" and indicators == "RSI":
    st.plotly_chart(close_chart(new_df1, num_period), key="chart5")
    st.plotly_chart(RSI(new_df1, num_period), key="chart6")

elif chart_type == "Line" and indicators == "MACD":
    st.plotly_chart(close_chart(new_df1, num_period), key="chart7")
    st.plotly_chart(MACD(new_df1, num_period), key="chart8")

elif chart_type == "Line" and indicators == "MOVING AVERAGE":
    st.plotly_chart(Moving_average_stock(new_df1, num_period), key="chart9")